...
def save_visuals(self,exp_folder,iter_step,rx=0,ry=0,rz=0,tz=0):
    ...
    save_tensor2img(os.path.join(exp_folder,f'{iter_step}_deformation.png'), self.deformation_map * 30)  # 乘以20是为了可视化
    # 固定视角下的渲染
...
def save_results(self,exp_folder,iter_step,save_mesh=True,save_npy=True):
    ...
    if global_config.save_deformation_map_for_test:
        np.save(os.path.join(exp_folder,f'{iter_step}_deformation.npy'),self.deformation_para.detach().cpu().numpy())
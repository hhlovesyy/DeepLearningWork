...
class StageFitter(object):
    def __init__(self, SD_guidance,
        ...
        self.facemodel = HIFIParametricFaceModel(
                hifi_folder='/root/autodl-tmp/HIFI3D/files', camera_distance=self.camera_d, focal=self.focal, center=center,
                is_train=True, 
                opt_id_dim = 526,
                opt_exp_dim = 203,
                opt_tex_dim = 80,
        ...
        with torch.no_grad():
            ...
            if (self.stage == 'deformation map' or self.stage == 'displacement map') and global_config.test_texture_first:
                # 如果不是id->texture->deformation map->displacement map的顺序，那么就不需要加载diffuse，否则需要加载diffuse
                self.load_diffuse(saved_diffuse_path)
    ...
    def init_parameters(self):
        ...
        self.deformation_para = nn.Parameter(torch.zeros(1, global_config.deformation_map_resolution, global_config.deformation_map_resolution, 3).float().to(self.device))

...
class HIFIParametricFaceModel:
    def __init__(self, 
        ...
        if not os.path.isfile(os.path.join(hifi_folder, default_shape_name)):
            print(os.path.join(hifi_folder, default_shape_name))
    ...
    def compute_for_render_with_deform_map(self, coeffs, deform_map, use_external_exp=False):
        print("now we are in compute_for_render_with_deform_map")
        coeff_dict = self.split_coeff(coeffs)
        if not use_external_exp:
            # print("not use external exp")
            # print(coeff_dict['exp'].shape)  # torch.Size([1, 203]) # exp指的是表情的系数
        ...
        face_shape_dp = self.update_vertex_by_deformation_map(face_shape,deform_map)
    ...
    def compute_for_render_with_dp_map(self, coeffs, dp_map, use_external_exp=False):
        print("now we are in compute_for_render_with_dp_map")
        coeff_dict = self.split_coeff(coeffs)
        if not use_external_exp:
            # print("not use external exp")
            # print(coeff_dict['exp'].shape)  # torch.Size([1, 203]) # exp指的是表情的系数
    ...
    def update_vertex_by_deformation_map(self, vertex, deformation_map):
        vertex_vt = self.vt.view(1,-1,1,2).contiguous()
        vertex_deformation_xyz = dr.texture(deformation_map.flip(1,),vertex_vt[:,self.v2vt.int(),:,:]).view(1,-1,3).contiguous()
        # print("========vertex_deformation_xyz.shape===========")
        # print(vertex_deformation_xyz.shape)  # torch.Size([1, 20481, 3])
        # print("===============================================")
        vertex_offset = vertex_deformation_xyz

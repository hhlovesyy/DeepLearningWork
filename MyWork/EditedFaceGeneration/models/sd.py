...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None,controlnet_name=None):
        ...
        if global_config.fix_SD and global_config.use_controlet_for_displacement:
            self.sd_version = '1.5' 
        ...
        if not global_config.use_controlet_for_displacement or not global_config.use_controlnet_for_deformation:
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100,control_img=None):
        ...
        with torch.no_grad():
            ...
            if self.use_controlnet:
                ...
                # print("======================================")
                # # displacement map: torch.Size([1, 3, 512, 512]) torch.Size([2, 4, 64, 64]) torch.Size([2, 77, 1024])  # todo:由于controlnet 的限制，text_embedding只能用sd 1.5，但应该问题不大，后面只要把text_embedding换成对应版本的就行
                # print(control_img.shape, control_model_input.shape, controlnet_prompt_embeds.shape)
                # print("======================================")

...
def train_step(fitter, prompt, text_z, static_text_z, opt,
    ...
    # 如果是几何阶段
    if fitter.stage in ['coarse geometry generation', 'deformation map', 'displacement map']:
        # 进行forward，得到渲染图
        rendered, grey_rendered, depth, norm, mask = fitter.forward()
        # normalize 深度
        norm_depth = fitter.normalize_depth(depth)
        # 按渲染方式选择loss计算的输入
        if global_config.test_texture_first:
            select_input_type = 'rendered'
    ...
    with torch.cuda.amp.autocast(enabled=True):
        ...
        if fitter.stage == 'deformation map':
            if global_config.use_controlnet_for_deformation:
                control_img = None
...
def main():
    ...
    print("test deformation with Mark Elliot Zuckerberg")
    ...
    elif global_config.test_deformation_map:
    ...
    if opt.stage == 'deformation map':
        if global_config.use_controlnet_for_deformation:
            exp_folder = os.path.join(exp_folder,'controlnet_def')
        else:
            exp_folder = os.path.join(exp_folder,"deformation_res")

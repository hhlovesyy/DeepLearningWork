...
def train_step(fitter, prompt, text_z, static_text_z, opt,
    ...
    if fitter.stage == 'edit':
        ...
        print("selected type is:", select_input_type)
        ...
        elif opt.edit_scope == 'deformation' or opt.edit_scope == 'displacement':
            if select_input_type == 'rendered':
                # print("successfully set loss input and ip2p_condition_img")
                # print("loss input shape:", loss_input.shape)  # torch.Size([1, 3, 224, 224])
                # print("ip2p_condition_img shape:", ip2p_condition_img.shape) # torch.Size([1, 3, 224, 224])
                loss_input = rendered
                ip2p_condition_img = fitter.pred_face_origin_diffuse.detach().clone()
                save_tensor2img(os.path.join('/root/FaceGeneration/FaceG2E/tmpDump',f'iter_{iter_step}_loss_input.png'),loss_input)
                save_tensor2img(os.path.join('/root/FaceGeneration/FaceG2E/tmpDump',f'iter_{iter_step}_ip2p_condition_img.png'),ip2p_condition_img)
    ...
    with torch.cuda.amp.autocast(enabled=True):
        ...
        if fitter.stage == 'edit':
            if opt.attention_sds:  # 在SDS阶段使用attention，设置里一般为true
    ...
    if fitter.stage == 'edit':
        ...
        if opt.edit_scope == 'tex':
            ...
            # todo:这里是一致性损失，后面感觉要额外进行设计，先用geo部分的抄过来看看
            if not hasattr(fitter,'origin_pos_map'):
                fitter.origin_pos_map = fitter.shape2posmap(fitter.load_vertex[0])
            fitter.update_pos_map = fitter.shape2posmap(fitter.pred_vertex_no_pose[0])
            reg_consistency = (fitter.UV_attention_weight * (fitter.update_pos_map - fitter.origin_pos_map) ** 2).mean()
        ...
        print(f'iter_step: {iter_step}, reg_consistency: {reg_consistency.item()}')
        loss += opt.w_reg_diffuse * reg_consistency

    print(f'iter_step: {iter_step}, loss: {loss.item()}')
    return loss

def plot_loss_curve(losses, exp_folder):
...
def main():
    ...
    if opt.force_fixed_viewpoint:
        ...
        if opt.stage == 'edit':
            ...
            if opt.edit_scope == 'deformation' or opt.edit_scope == 'displacement':
                fitter.set_transformation_choices(x_list=[0,-10,-20],y_list=[0,60,300,30,330],z_list=[0],t_z_list=[1.5])   
    ...
    # if global_config.test_deformation_map:
    #     exp_folder = os.path.join(exp_root,exp_name,'test_deformation_map',opt.stage,f'seed{seed_num}')
    # elif global_config.test_displacement_map:
    #     exp_folder = os.path.join(exp_root,exp_name,'test_displacement_map',opt.stage,f'seed{seed_num}')
    exp_folder = os.path.join(exp_root,global_config.test_fileroot)
